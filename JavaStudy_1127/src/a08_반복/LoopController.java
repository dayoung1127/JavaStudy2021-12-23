...
// while(true) , for(true) 는 무한반복
// boolean xxx = true; 를 false로 변경하면 반복문이 멈춤
// int i = 0; <- for문 안에서 변수 사용 가능
// for(int i=0;) <- for문 밖에서는 변수 쓸 수 없음
...
public class LoopController {
	...
	public static void main(String[] args) {
		...
		int cmd;
		while(loopFlag) {		//false가 될때까지 무한반복
			...
			if(cmd == 0) {
				...
				while(true) {
					...
					if(cmd == 0) {
						System.out.println("프로그램 종료중...");
						loopFlag = false;   // 나가서 맨 위 while(loopflag)로[바같쪽loop] 이동 -> 
						                    // false로 변경되었기 때문에 반복 없이 종료
						break;
					} else if(cmd == 9) {
						System.out.println("이전 페이지로 이동합니다.");
						break; //바깥쪽 while로 돌아감
					} else if(cmd == 1) {
